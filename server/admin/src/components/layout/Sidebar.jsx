      ...
      >
        <div className="h-header flex items-center justify-between px-5 border-b border-border-main shrink-0">
          <div className="flex flex-col leading-tight">
            <span className="font-serif font-semibold text-text-main text-lg">TeioOS</span>
        ...
        <nav aria-label="Primary" className="flex-1 overflow-y-auto px-3 py-4">
          <ul className="space-y-6">
            {visibleGroups.map((group) => (
              <li key={group.label}>
                <p className="px-2.5 mb-1.5 text-caption font-medium text-text-muted">
                  {group.label}
                </p>
                    ...
                    return (
                      <li key={item.path}>
                        ...
                        >
                          ...
                          {item.isPlanned && (
                            <span
                              className="text-[10px] font-medium text-text-muted bg-subtle border border-border-main rounded px-1.5 py-0.5"

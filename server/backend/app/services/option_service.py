...
class OptionService:
    ...
    def _validate_question(self, question_id: UUID) -> None:
    ...
    def create_option(self, data: OptionCreate) -> Option:
        self._validate_question(data.question_id)
    ...
    def update_option(self, option_id: UUID, data: OptionUpdate) -> Option:
        ...
        if data.question_id and data.question_id != option.question_id:
            self._validate_question(data.question_id)
            
        # Handle logic for multiple correct options
        if data.is_correct is True:
            # If it wasn't correct before, or if it was but we are just making sure

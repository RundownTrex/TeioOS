...
export const AutoSaveIndicator = ({
  ...
  const configs = {
    ...
    [SYNC_STATUS.LOCAL]: {
      ...
      icon: <CloudOff className="w-3.5 h-3.5 text-amber-600" />,
  ...
  return (
    <span
      role="status"
      aria-live="polite"
      className={`inline-flex items-center gap-1.5 text-xs font-sans select-none ${config.colorClass} ${className}`}
    >
      {config.icon}

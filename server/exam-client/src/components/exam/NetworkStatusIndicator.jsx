  ...
  return (
    <span
      ...
      className={`inline-flex items-center gap-1.5 text-xs font-mono leading-none select-none ${
        isConnected ? 'text-text-muted' : 'text-status-danger font-bold'
      } ${className}`}
    >
      {isConnected ? (
        <>
          <span className="shrink-0 flex items-center justify-center">
            <Wifi className="w-3.5 h-3.5 text-status-success shrink-0" aria-hidden="true" />
          </span>
        ...
        <>
          <span className="shrink-0 flex items-center justify-center">
            <WifiOff className="w-3.5 h-3.5 text-status-danger animate-pulse shrink-0" aria-hidden="true" />
          </span>

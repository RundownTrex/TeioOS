...
export const QuestionTile = ({
  ...
  const statusStyles = {
    ANSWERED:
      'bg-status-answered-bg text-status-answered border-status-answered font-medium hover:brightness-95',
    UNANSWERED:
      'bg-status-unanswered-bg text-status-unanswered border-status-unanswered font-medium hover:brightness-95',
    REVIEW:
      'bg-status-review-bg text-status-review border-status-review font-medium hover:brightness-95',
    UNVISITED:
  ...
  return (
    <button
      ...
      {...rest}
      className={`relative w-10 h-10 rounded border flex items-center justify-center font-mono text-xs font-medium transition-colors duration-fast select-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-navy-primary focus-visible:ring-offset-1 active:scale-95 ${
        statusStyles[status] || statusStyles.UNVISITED
      } ${
        isActive
          ? 'ring-2 ring-navy-primary ring-offset-1 ring-offset-surface border-navy-primary font-bold'
    ...
    >
      ...
      {status !== 'UNVISITED' && (
        <span className="absolute -top-1.5 -right-1.5 bg-surface border border-border-main rounded-full w-4 h-4 flex items-center justify-center">
          {renderBadgeIcon()}
        </span>
      )}
    </button>

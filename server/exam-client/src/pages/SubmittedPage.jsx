...
export const SubmittedPage = () => {
  ...
  useEffect(() => {
    registerHandler('dashboardStartExam', () => navigate('/dashboard'));
  ...
  }, [registerHandler, unregisterHandler, navigate, logout]);
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'Enter' || e.key === ' ' || key === 'D' || e.key === 'Escape') {
        e.preventDefault();
        navigate('/dashboard');
      } else if (key === 'L') {
        e.preventDefault();
  ...
  useEffect(() => {
    const prompt = 'Examination submitted successfully. Press Enter to return to your Dashboard, or press L to Log Out.';
      ...
      <div className="max-w-[600px] mx-auto space-y-6 select-none my-4 sm:my-8">
        ...
        <Card className="border border-border-main bg-surface overflow-hidden">
          ...
          <CardHeader className="text-center py-8 bg-subtle/40 border-b border-border-main">
            ...
            <p className="text-caption text-text-muted mt-1.5 leading-relaxed max-w-md mx-auto">
          ...
          <CardBody className="space-y-6 p-6">
            ...
            <section aria-labelledby="submission-details-heading" className="space-y-3">
              ...
              <div className="p-4 border border-border-main bg-subtle/50 rounded space-y-2.5 text-xs font-mono">
            ...
            <section aria-labelledby="room-guidance-heading" className="space-y-3">
              <div className="flex items-start gap-3 p-4 bg-navy-tint/40 border border-navy-primary/20 text-text-main text-xs rounded font-medium leading-relaxed">
                <ShieldCheck className="w-5 h-5 shrink-0 text-navy-primary mt-0.5" aria-hidden="true" />
                <div className="space-y-1">
                  <h3 id="room-guidance-heading" className="text-xs font-semibold text-text-main">
